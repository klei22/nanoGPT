...
`[num_starts, block_size, vocab_size]`, where `num_starts = len(split.bin) - block_size`,
aligned with the `train.bin`/`val.bin` token order and the configured block size.
