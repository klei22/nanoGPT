...
def camera_and_logic_loop():
    ...
    while True:
        ...
        with lock:
            if show_grayscale:
                # 🛠️ FIXED: Clean downsampled matrix mode without guide lines polluting the view
                current_frame = cv2.resize(small_frame, (640, 360), interpolation=cv2.INTER_NEAREST)
                # Convert back to BGR color space block shape so web compiler can push stream bytes stably
            ...
            <script>
                ...
                function toggleViewMode() {
                    fetch('/toggle_view').then(res => res.text()).then(txt => {
                        ...
                        // 🛠️ FIXED: Toggle D-Pad opacity and link pointers out of matrix frame space
                        ...
                        if (txt.includes("Matrix")) {
                            ...
                            dpad.style.pointerEvents = "none"; // Disables touch interactions when hidden
                        }
                    });
                }
