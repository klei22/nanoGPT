...
vacuum_on = False
last_keys_set = set()
auto_mode = False       # True for Blue Line Follower
roam_mode = False       # True for Random Roam Mode
...
def camera_and_logic_loop():
    global current_frame, auto_mode, roam_mode, show_grayscale, current_action_label, csv_file, SAMPLE_SIZE, current_speed, active_key_string
    ...
    csv_file.write(f"frame_number,timestamp,action,pressed_keys,bump_left,bump_right,wheel_dropped,total_distance_mm,heading_deg,speed_mm_s,left_wheel_v,right_wheel_v,battery_percent,{pixel_headers}\n")
    ...
    while True:
        ...
        log_keys = "" if active_key_string == "space" else active_key_string
        log_action = "" if current_action_label == "stop" else current_action_label
        
        csv_file.write(f"{frame_number},{synced_time},{log_action},{log_keys},{b_left},{b_right},{w_drop},{total_distance:.1f},{current_heading:.1f},{live_speed_mms:.1f},{live_left_speed_mms},{live_right_speed_mms},{battery_pct:.1f},{pixel_str}\n")
        ...
        # --- AUTO MODE 1: LINE FOLLOWER WITH CLOSED-LOOP ENCODER RECOVERY ---
        if auto_mode:
        ...
        # --- AUTO MODE 2: RANDOM ROAM MODE ---
        elif roam_mode:
            if b_left == 1 or b_right == 1:
                current_action_label = "obstacle_recoil"
                active_key_string = "s"
                drive_roomba(-120, 32767)  # Back up slightly
                time.sleep(0.5)
                # Spin around and pick an alternate direction
                execute_closed_loop_turn(random.randint(90, 220), random.choice([1, -1]))
            else:
                current_action_label = "random_roaming"
                active_key_string = "w"
                drive_roomba(current_speed, 32767)  # Proceed straight forward ahead
    ...
    <html>
        <head>
            ...
            <style>
                ...
                .hud-btn-active-roam { border-color: #ffaa00; color: #ffaa00; font-weight: bold; }
            ...
            <script>
                ...
                function toggleAutoMode() {
                    fetch('/toggle_auto').then(res => res.text()).then(txt => { 
                        ...
                        roamBtn.classList.remove("hud-btn-active-roam");
                        if (txt.includes("LINE")) { autoBtn.classList.add("hud-btn-active-auto"); } 
                ...
                function toggleRoamMode() {
                    fetch('/toggle_roam').then(res => res.text()).then(txt => { 
                        ...
                        if (txt.includes("ROAM")) { roamBtn.classList.add("hud-btn-active-roam"); } 
                        else { roamBtn.classList.remove("hud-btn-active-roam"); }
                    });
                }
                ...
                function triggerDocking() {
                    fetch('/dock').then(res => res.text()).then(txt => {
                        ...
                        document.getElementById("hud-roam-btn").classList.remove("hud-btn-active-roam");
        ...
        <body>
            ...
            <div id="hud-root" class="video-hud-container">
                ...
                <div class="hud-controls-box">
                    ...
                    <button class="hud-btn class hud-btn-degree" onclick="triggerFixedDegreeTurn(90, 'left')">📐 Turn Left 90°</button>
                    ...
                    <button id="hud-roam-btn" class="hud-btn" style="border-color:#ffaa00; color:#ffaa00;" onclick="toggleRoamMode()">Random Roam Mode</button>
...
def toggle_auto():
    ...
    current_action_label = "stop"
    active_key_string = "space"
    return "LINE FOLLOWING MODE" if auto_mode else "MANUAL MODE"
...
def toggle_roam():
    ...
    current_action_label = "stop"
    active_key_string = "space"
    return "ROAMING MODE ACTIVE" if roam_mode else "MANUAL MODE"

@app.route('/advance_macro_mode')
def advance_macro_mode():
    global operational_mode, movement_history, current_move_start, last_saved_keys, auto_mode, roam_mode, playback_index, active_key_string
    auto_mode = False
    roam_mode = False
...
def trigger_sticky_spin():
    global current_action_label, active_key_string, auto_mode, roam_mode
    auto_mode = False
    roam_mode = False
...
@app.route('/dock')
def dock():
    global auto_mode, roam_mode, operational_mode, current_action_label, vacuum_on, active_key_string
    auto_mode = False
    roam_mode = False
    operational_mode = "manual"
    vacuum_on = False 
...
def keyboard_input():
    ...
    if not auto_mode and not roam_mode and operational_mode != "playback" and operational_mode != "paused" and current_action_label != "precision_turning" and "encoder_turning" not in current_action_label and current_action_label != "docking" and "sticky" not in current_action_label:
