...
from scripts.benchmark_hf_attention_speed import format_table
...
def test_attention_benchmark_table_contains_all_variations():
    results = [
        {"variant": variant, "average_ms": index + 1.0,
         "iterations_per_second": 1000.0 / (index + 1), "tokens_per_second": 10.0,
         "peak_memory_gib": 1.0}
        for index, variant in enumerate(
            ("relu2max_torch", "relu2max_triton", "softmax_generic", "softmax_sdpa")
        )
    ]
    table = format_table(results)
    assert "ReLU2Max (PyTorch)" in table
    assert "ReLU2Max (Triton)" in table
    assert "Softmax (generic)" in table
    assert "Softmax (SDPA/conventional)" in table
