...
def test_full_with_wte_adds_embedding_as_separate_source():
    config = GPTConfig(
        block_size=4,
        vocab_size=32,
        n_layer=2,
        ...
        attention_residual_variant="full_with_wte",
    )
    model = GPT(config)
    source_counts = []
    original_forward = model.attention_residual.forward

    def record_sources(sources, destination):
        source_counts.append(len(sources))
        return original_forward(sources, destination)

    model.attention_residual.forward = record_sources
    tokens = torch.randint(0, config.vocab_size, (2, config.block_size))
    targets = torch.randint(0, config.vocab_size, (2, config.block_size))
    ...
    assert source_counts == [2, 3, 4, 5, 6]
