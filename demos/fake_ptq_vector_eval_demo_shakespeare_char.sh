...
#
# Sweeps fake post-training quantization across per-vector and per-tensor
# granularities on shakespeare_char, evaluates validation loss with optional
# embedding Gaussian noise, and plots validation loss + angle deltas vs bits.

set -euo pipefail

DATASET="shakespeare_char"
EVAL_DATASET_DIR="data/${DATASET}"
OUT_DIR="out_fake_ptq_${DATASET}"
...
echo "Embedding noise std sweep: ${NOISE_LEVELS[*]}"
...
echo "=== Step 2: Train a reference model on ${DATASET} (if needed) ==="
...
def noise_tag(noise: float) -> str:
    return f"noise_{noise:.2f}".replace(".", "p")
...
entries = []
for granularity, root in [("vector", vector_root), ("tensor", tensor_root)]:
        ...
        writer.writerow(
            {
                ...
                "noise_std": f"{entry['noise_std']:.2f}",
...
for granularity in ("vector", "tensor"):
    ...
    for noise in noise_levels_sorted:
        ...
        label = f"{granularity} noise={noise:.2f}"
        ax_loss.plot(
            bits,
            losses,
