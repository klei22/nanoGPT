#!/usr/bin/env bash
# Integer-quantized CSV numerical multicontext demo using
# learned_vector_scaled_arc embedding variant:
#   Each channel learns a base vector on the embedding hypersphere and an
#   arc direction.  The scalar input controls the angle of rotation along
#   the great-circle arc, producing the channel embedding.
#
# 1) build per-column integer datasets via shift/scale/round/clip
# 2) train numerical multicontext model with learned_vector_scaled_arc
...
python3 train.py \
  ...
  --numerical_embedding_variant learned_vector_scaled_arc \
  ...
  --dtype bfloat16 \
  --compile \
  --out_dir out/numerical_mc_csv_int_lvsa
...
python3 sample.py \
  ...
  --numerical_multicontext_plotly_file out/numerical_mc_csv_int_lvsa/num_mc_csv_int_lvsa_samples.html \
  --max_new_tokens 256 \
  --num_samples 1
