...
class Grad:
    ...
    y: float
    z: float
    def dot2(self, x: float, y: float) -> float:
...
class NoiseGenerator:
    def __init__(self, seed: int):
        self.grad_p = [Grad(0.0,0.0,0.0)]*512
        self.perm = [0]*512
        self.set_seed(seed)
    ...
    def set_seed(self, seed: int):
        ...
        for i in range(256):
            ...
            new_grad = GRAD3[v % 12]
            self.grad_p[i] = self.grad_p[i+256] = new_grad

    def simplex(self, x: float) -> float:
        return self.simplex2(x*1.2, -x*0.7)

    def simplex2(self, xin: float, yin: float) -> float:
        ...
        ii = i & 255
        jj = j & 255
        gi0 = self.grad_p[ii + self.perm[jj]]
        gi1 = self.grad_p[ii+i1 + self.perm[jj+j1]]
        gi2 = self.grad_p[ii+1 + self.perm[jj+1]]
        t0 = 0.5 - x0*x0 - y0*y0
        n0 = 0.0
        if t0 >=0:
            t0 *= t0
