...
from variations.numerical_mapping_variations import get_numerical_mapping
    ...
    def __init__(self, config):
        ...
        if self.uses_numerical_multicontext:
            ...
            for idx in range(len(config.vocab_sizes)):
                key = str(idx)
                self.numerical_embeddings[key] = get_numerical_mapping(
                    config.numerical_embedding_mapping,
                    config,
                    in_dim=1,
                    out_dim=config.n_embd,
                )
                self.numerical_output_mlps[key] = get_numerical_mapping(
                    config.numerical_output_mapping,
                    config,
