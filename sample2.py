...
def parse_args():
    ...
    parser.add_argument('--stats_folder', type=str, default=None, help="optional folder for statistics")

    return parser.parse_args()


def save_chart(probs, idx, decode, step, out_dir, last_k_tokens, chart_type, selected_token, stats_folder, start_token):
    ...
    if stats_folder:
        ...
        stats_file = os.path.join(stats_folder, "stats.json")
        start_token_escaped = start_token.replace('\n', '\\n').replace('\t', '\\t')
        token_probs = {top_k_tokens[i]: top_k_probs[0, i].item() for i in range(len(top_k_tokens))}
        entry = {
            "start_token": start_token_escaped,
            "step": step,
                ...
                for k in range(args.num_samples):
                    ...
                    for step in range(args.max_new_tokens):
                        ...
                        if args.show_heatmaps:
                            ...
                            save_chart(probs, x, decode, step, out_dir, args.last_k_tokens, args.chart_type, selected_token, args.stats_folder, args.start)
