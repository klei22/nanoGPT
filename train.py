            ...
            for split in ['train', 'val']:
                ...
                for k in range(self.args.eval_iters):
                    ...
                    if split == 'val':
                        ...
                        target_logits = logits.gather(-1, Y.unsqueeze(-1)).squeeze(-1)
                        target_prob = probs.gather(-1, Y.unsqueeze(-1)).squeeze(-1).float()
                        if self.args.chunked_rank_eval:
                            # Chunked path: avoids a full [batch, seq, vocab_size]
                            # intermediate tensor (needed when vocab_size is large).
                            _chunk = self.args.rank_eval_vocab_chunk_size
                            _V = logits.size(-1)
                            ranks = torch.zeros(
                            ...
                            for _c in range(0, _V, _chunk):
                                _lg = logits[..., _c:_c + _chunk]
                                ranks += (_lg > tgt_l).sum(dim=-1)
                                _pr = probs[..., _c:_c + _chunk].float()
