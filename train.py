...
class Trainer:
    ...
    def __init__(self, args, model_group, training_group, logging_group):
        ...
        self.latest_avg_prob_rank = float('nan')
    ...
    def estimate_loss(self):
        ...
        def avg_prob_rank(prob_list: list[torch.Tensor]) -> torch.Tensor:
            """Return rank implied by the mean probability or NaN when undefined."""
            if not prob_list:
                return torch.tensor(float('nan'))
            mean_prob = torch.cat(prob_list).mean()
            if torch.isfinite(mean_prob) and mean_prob > 0:
                return torch.reciprocal(mean_prob)
            return torch.tensor(float('nan'))
        ...
        if self.args.dataset_list:
            for dataset in self.args.dataset_list:
                ...
                top1_probs, top1_corrects, target_ranks, target_probs, target_left_probs = [], [], [], [], []
                ln_f_cosines = []
                ...
                out['datasets'][dataset] = {
                        ...
                        'avg_prob_rank': avg_prob_rank(target_probs),
            ...
            out['avg_prob_rank'] = out['datasets'][self.args.dataset]['avg_prob_rank']
            ...
            for split in ['train', 'val']:
                ...
                if split == 'val':
                    ...
                    out['avg_prob_rank'] = avg_prob_rank(target_probs)
    ...
    def log_metrics(self, losses, running_mfu, epoch, tokens_trained, target_dataset, val_better_than_chance):
        ...
        if self.args.tensorboard_log:
            ...
            if 'top1_prob' in losses:
                ...
                self.writer.add_scalar(f"{target_dataset}/avg_prob_rank", losses['avg_prob_rank'], self.iter_num)
    ...
    def train(self):
        ...
        progress = Progress(
                ...
                TextColumn("[bold dark_magenta]AvgProbRank:[/bold dark_magenta]{task.fields[avg_prob_rank]}"),
        ...
        with Live(Group(progress.get_renderable(), cli_text), console=self.console, refresh_per_second=10) as live:
            task_id = progress.add_task(
                    ...
                    avg_prob_rank=f"{self.latest_avg_prob_rank:.6f}",
            ...
            while True:
                ...
                if self.iter_num % self.args.eval_interval == 0 and self.master_process:
                    ...
                    self.latest_avg_prob_rank = losses.get('avg_prob_rank', float('nan'))
                        ...
                        if losses['val'] < self.best_val_loss:
                            ...
                            with open(os.path.join(self.args.out_dir, 'best_val_loss_and_iter.txt'), "w") as best_loss_file:
                                ...
                                metrics = [
                                        ...
                                        f"{losses.get('avg_prob_rank', float('nan')):.6f}",
