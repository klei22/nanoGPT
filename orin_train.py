...
class Trainer:
    ...
    def load_data(self):
        def _dataset_bin_dtype(dataset_name):
            # Ported from train.py: the .bin files are written as uint16
            # unless vocab_size exceeds what uint16 can hold, in which case
            # they're written as uint32. Reading with the wrong dtype
            # silently wraps token IDs instead of erroring, so this must
            # match whatever prepare.py used when the .bin was created.
