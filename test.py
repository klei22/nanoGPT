...
def factorize_matrix(A, output_dir):
    ...
    np.save(f"{output_dir}/W1.npy", W1.detach().numpy())
    np.save(f"{output_dir}/W2.npy", W2.detach().numpy())
...
def run_experiment_with_vizier(vizier_algorithm, vizier_iterations):
    ...
    for i in range(vizier_iterations):
        ...
        for suggestion in suggestions:
            ...
            A = params["A"]
            loss = factorize_matrix(A, f"output_{i}_A_{A}")
...
def main():
    ...
    run_experiment_with_vizier(vizier_algorithm, vizier_iterations)
