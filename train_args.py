...
def parse_args():
    ...
    training_group.add_argument('--colorize_mode', type=str, default='minmax', choices=['minmax', 'softmax', 'softmax_top_k', 'rank', 'router', 'all'], help="Colorization mode for tokens (see sample.py).")
    ...
    model_group.add_argument('--use_output_router', default=False, action=argparse.BooleanOptionalAction, help='enable routing over per-layer MLP outputs before final layernorm')
    model_group.add_argument('--output_router_top_k', default=2, type=int, help='number of MLP outputs to mix during training when using the output router')
    model_group.add_argument('--output_router_eval_top_k', default=1, type=int, help='number of MLP outputs to mix during evaluation when using the output router')
