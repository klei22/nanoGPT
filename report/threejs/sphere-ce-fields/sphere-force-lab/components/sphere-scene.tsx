...
export type SphereSceneProps = {
  ...
  showHiddenOnSphere: boolean;
  showHiddenInSpace: boolean;
...
function targetColor(index: number, targetCount: number) {
  if (targetCount <= tokenColors.length) return tokenColors[index % tokenColors.length];
  return new THREE.Color().setHSL((0.53 + (index / Math.max(1, targetCount)) * 0.72) % 1, 0.82, 0.68).getHex();
}
...
function labelSprite(text: string, color: string, isLetter: boolean) {
  ...
  ctx.fill();
  ctx.stroke();
  const fontSize = text.length <= 2 ? 58 : text.length === 3 ? 42 : 34;
...
function hiddenLabel(text: string, color: string) {
  const canvas = document.createElement("canvas");
  ...
  ctx.fillText(text, 96, 33);
  const texture = new THREE.CanvasTexture(canvas);
  texture.colorSpace = THREE.SRGBColorSpace;
  const label = new THREE.Sprite(new THREE.SpriteMaterial({ map: texture, transparent: true, depthTest: false, fog: false }));
  label.renderOrder = 12;
  return label;
}

function addGreatCircle(scene: THREE.Scene, plane: "xy" | "xz" | "yz", RADIUS: number) {
...
export function SphereScene(props: SphereSceneProps) {
  ...
  useEffect(() => {
    ...
    scene.add(key);
    addGreatCircle(scene, "xy", RADIUS);
    if (snapshot.modelDim >= 3) { addGreatCircle(scene, "xz", RADIUS); addGreatCircle(scene, "yz", RADIUS); }
    const sampleCount = propsRef.current.snapshot.logPartition.length;
    const widthSegments = sampleCount > 5000 ? 28 : sampleCount > 2000 ? 36 : sampleCount > 500 ? 48 : 64;
    const heightSegments = sampleCount > 5000 ? 18 : sampleCount > 2000 ? 22 : sampleCount > 500 ? 30 : 40;
    ...
    sphere.visible = snapshot.modelDim >= 3;
    ...
    const resize = () => {
      ...
      const halfHeight = Math.max(2.65, viewExtent * 1.35) / Math.min(1, aspect);
      camera.left = -halfHeight * aspect;
      camera.right = halfHeight * aspect;
      camera.top = halfHeight;
      camera.bottom = -halfHeight;
      camera.far = Math.max(100, viewExtent * 8);
      camera.updateProjectionMatrix();
    };
    const fitExtent = (extent: number) => {
      viewExtent = extent;
      camera.position.setLength(Math.max(8, extent * 3));
      resize();
    };
    stateRef.current = { scene, camera, renderer, controls, sphere, dynamic, hover, raycaster, pointer, fitExtent };
    ...
    const projectPointer = (event: PointerEvent) => {
      ...
      if (snapshot.modelDim === 2) {
        ...
        return point && point.length() > 1e-9 && point.length() <= RADIUS * 1.1 ? point.normalize().multiplyScalar(RADIUS) : null;
      }
    ...
    const pointerMove = (event: PointerEvent) => {
      ...
      if (point) hover.position.copy(point.clone().normalize().multiplyScalar(RADIUS));
    };
  ...
  useEffect(() => {
    ...
    if (heatmapVisible) {
      for (let i = 0; i < positions.count; i += 1) {
        ...
        values.push(
          probeMode === "force" ? probe.magnitude :
          probeMode === "potential" ? probe.potential :
          probeMode === "probability" ? probe.probability : probe.radial,
        );
      }
    }
    let range = fixedRange(probeMode);
    if (heatmapVisible && frameAutoscale && values.length) {
    ...
    if (showTrails) {
      for (let token = 0; token < snapshot.activeVocab; token += 1) {
        ...
        if (points.length > 1) {
          ...
          const color = targeted ? targetColor(token, snapshot.targetCount) : 0xffc95d;
          const line = new THREE.Line(geometry, new THREE.LineBasicMaterial({ color, transparent: true, opacity: targeted ? 0.42 : 0.72 }));
          line.renderOrder = 3;
    ...
    for (let token = 0; token < snapshot.activeVocab; token += 1) {
      ...
      const isUntargeted = !snapshot.targetMask[token];
    ...
    if (showHiddenInSpace || showHiddenOnSphere) {
      ...
      for (const item of snapshot.hiddenMeans) {
        ...
        const projected = item.projected ? new THREE.Vector3(...item.projected) : null;
        if (showHiddenInSpace && showHiddenOnSphere && projected) {
          const link = new THREE.Line(new THREE.BufferGeometry().setFromPoints([raw, projected]), new THREE.LineDashedMaterial({ color, dashSize: 0.06 * displayScale, gapSize: 0.04 * displayScale, transparent: true, opacity: 0.65, depthTest: false, fog: false }));
          link.computeLineDistances(); link.renderOrder = 9; state.dynamic.add(link);
        }
        const drawMean = (point: THREE.Vector3, onSphere: boolean) => {
          const marker = new THREE.Mesh(new THREE.OctahedronGeometry(0.085 * displayScale), new THREE.MeshBasicMaterial({ color, wireframe: onSphere, depthTest: false, fog: false }));
          marker.position.copy(point); marker.renderOrder = 11;
          marker.userData = { target: item.target, hiddenMean: true, projected: onSphere, sampleCount: item.count };
          state.dynamic.add(marker);
          if (snapshot.hiddenMeans.length <= 24 || item.target === selectedToken || item.target % Math.ceil(snapshot.hiddenMeans.length / 24) === 0) {
            const label = hiddenLabel(`μ${item.target}${onSphere ? "ˢ" : ""}`, new THREE.Color(color).getStyle());
            label.scale.set(0.46 * displayScale, 0.155 * displayScale, 1);
            label.position.copy(point).add(new THREE.Vector3(0, (onSphere ? 0.18 : -0.18) * displayScale, 0));
            state.dynamic.add(label);
          }
        };
    ...
    if (mode === "decomposition" && snapshot.activeVocab > selectedToken) {
      const o = selectedToken * D_MODEL;
      const p = new THREE.Vector3(snapshot.positions[o], snapshot.positions[o + 1], snapshot.positions[o + 2]);
  ...
  return (
    <div className="sphere-canvas relative h-full min-h-[430px] w-full overflow-hidden rounded-[22px] bg-[#050914]" ref={mountRef}>
      <div className="sphere-hint pointer-events-none absolute left-4 top-4 z-10 rounded-full border border-white/10 bg-[#07101e]/82 px-3 py-1.5 text-[11px] font-medium tracking-wide text-slate-300 backdrop-blur">
        {compact ? (insertionAllowed ? "Tap the surface to place an untargeted row" : "Drag to orbit · pinch to zoom") : "Drag to orbit · scroll to zoom · click surface to add an untargeted row"}
      </div>
    </div>
  );
}
