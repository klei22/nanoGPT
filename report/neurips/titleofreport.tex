...
\section{Embedding-Based Mixture-of-Attention}
Our model combines an always-on set of shared attention heads with a routed mixture of query-specific heads that attend directly over the token embedding matrix. A router activates a sparse subset of these mixture-of-attention (MoA) heads for each query, enabling precomputation of the query--embedding products used during inference. Figure~\ref{fig:architecture} illustrates the data flow. This design extends standard self-attention~\cite{Vaswani2017} and draws inspiration from sparse expert routing~\cite{Shazeer2017}.

\begin{figure}[t]
  \centering
  \begin{tikzpicture}[node distance=1.6cm, >=stealth]
    \node (q) [draw, rounded corners] {Query};
    \node (router) [draw, rounded corners, right=of q] {Router};
    \node (shared) [draw, rounded corners, above right=1cm and 1.5cm of router] {Shared Heads};
    \node (moa) [draw, rounded corners, below right=1cm and 1.5cm of router] {MoA Heads};
    \node (embed) [draw, cylinder, shape border rotate=90, minimum height=1.5cm, right=of moa] {Embedding Table};
    \node (cproj) [draw, rounded corners, right of=shared, xshift=3cm] {Output Projection};
    \draw[->] (q) -- (router);
    \draw[->] (router) -- (shared);
    \draw[->] (router) -- (moa);
    \draw[->] (shared) -- (cproj);
    ...
    \draw[->] (embed) -- (cproj);
  \end{tikzpicture}
  \caption{MoA attention with shared heads and query-routed embedding heads.}
