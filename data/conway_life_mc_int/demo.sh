...
echo "Side-by-side comparison viewer URL: ${COMPARE_VIEWER_URL}"
if [[ "${OPEN_VIEWER}" -eq 1 ]]; then
  if command -v xdg-open >/dev/null 2>&1; then
    xdg-open "${VIEWER_URL}" >/dev/null 2>&1 || echo "Could not open browser automatically; paste one of the URLs above into a browser."
  elif command -v open >/dev/null 2>&1; then
    open "${VIEWER_URL}" >/dev/null 2>&1 || echo "Could not open browser automatically; paste one of the URLs above into a browser."
  else
    VIEWER_URL="${VIEWER_URL}" python3 - <<'PY' || echo "Could not open browser automatically; paste one of the URLs above into a browser."
import os
import webbrowser
...
PY
  ...
  echo "--no-open set; paste one of the URLs above into a browser."
fi
