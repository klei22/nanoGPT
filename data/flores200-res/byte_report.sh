...
for lang in "${lang_array[@]}"; do
  ...
  python3 prepare.py \
    ...
    -p 1.0 \
    --method json_byte_fallback \
    --json_tokens_file ../template/premade_vocab_sets/smollm2.json \
    --report_byte_tokenization
done

#REG QWEN
for lang in "${lang_array[@]}"; do
  python3 prepare.py -t "${lang}.txt" -p 1.0 --method json_byte_fallback --json_tokens_file ../template/premade_vocab_sets/qwen_tokens.json -s -S "$lang" --report_byte_tokenization
done
