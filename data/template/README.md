...
When `--max_parallel` is greater than 1, progress bars come only from each `prepare.py` subprocess (for example, `Tokenizing Bytes First + JSON Custom`). `batch_prepare.py` itself does not render additional progress bars.
