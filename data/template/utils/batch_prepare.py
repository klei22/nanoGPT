...
def _run_prepare_for_files(files, desc, max_parallel, command_builder):
    ...
    if max_parallel == 1:
        for command in tqdm(commands, desc=desc):
    ...
    def _worker_run(worker_index, worker_commands):
        worker_desc = f"{desc} [worker {worker_index + 1}]"
        bar = tqdm(total=len(worker_commands), desc=worker_desc, position=worker_index, leave=True)
        try:
            for command in worker_commands:
                _run_prepare_command(command)
                bar.update(1)
        except Exception as exc:  # pragma: no cover - concurrent error path
            with errors_lock:
                errors.append(exc)
        finally:
            bar.close()
    ...
    for worker_index, worker_commands in enumerate(worker_buckets):
        worker = threading.Thread(target=_worker_run, args=(worker_index, worker_commands), daemon=False)
