...
- `GET /api/layernorm/attention-sweep?layernorm=...&token_a=0&token_b=1` — paired A/B `dot(xnorm, xnorm WqWkᵀ)` values across every compatible layer and query head without constructing each full hidden-space operator (`token_id` remains a single-token compatibility parameter)
