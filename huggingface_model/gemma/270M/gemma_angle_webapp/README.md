...
- **All-head dot-product sweep.** Choose embedding A or B and evaluate `dot(xnorm, xnorm WqWkᵀ)` across every compatible layer and query head. The histogram can preserve model layer/head order or sort values from highest to lowest, with exact layer, query-head, KV-head, dot-product, and rank details on hover. The sweep associates the projection products without materializing a hidden-dimension-squared matrix per head.
- **All-norm A/B comparison.** Sweep every recognized block input norm and output norm against its corresponding attention layer/head, with embedding A and B shown as adjacent blue/pink bars in separate input-norm and output-norm histograms. Optionally repeat the final norm across every attention layer in both graphs. Natural numeric ordering is the default (`layer 9` before `layer 10`), with alphabetical ordering available for comparison.
...
- `GET /api/layernorm/attention-sweep?layernorm=...&token_id=0` — evaluate `dot(xnorm, xnorm WqWkᵀ)` for one token across every compatible layer and query head without constructing each full hidden-space operator
