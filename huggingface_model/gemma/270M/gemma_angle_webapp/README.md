...
- **Attention-head continuation.** Re-sort every channel plot by gain, A/B before, or A/B after values. Optionally choose a model layer and query head, apply its hidden-space `WqWkᵀ` operator to the complete selected norm output (RMS/centering, per-channel gain including Gemma's unit offset, and bias), and inspect two additional output plots. The combined gain × attention operator reports symmetry residual, symmetric/skew fractions, normalized distances to `+I` and `−I`, orthogonality/rotation residual, Frobenius norm, and per-embedding magnitude, participation ratio, angular changes, and `dot(norm_output, WqWkᵀ_output)`. Grouped-query attention maps each query head to its corresponding KV head.
- **Value/output-head continuation.** In the individual panel, switch the head operator to `WoWvᵀ` to plot `xnorm WoWvᵀ`, apply that block's post-attention output norm to the transformed branch, and plot the two resulting A/B output-norm vectors. The same option in the all-head sweep produces one paired A/B graph for `dot(xnorm, xnorm WoWvᵀ)` and a second paired graph for `dot(xnorm, attention_output_norm(xnorm WoWvᵀ))`.
- **All-head dot-product sweep.** Evaluate both embeddings A and B as adjacent blue/pink bars for `dot(xnorm, xnorm WqWkᵀ)` across every compatible layer and query head. Natural numeric order (`layer 9` before `layer 10`) is the default; switch to descending A, descending B, or alphabetical ordering. Exact layer, query-head, KV-head, paired dot products, and position are available on hover. The sweep associates the projection products without materializing a hidden-dimension-squared matrix per head.
- **All-norm A/B comparison.** Sweep every recognized block input norm and output norm against its corresponding attention layer/head, with embedding A and B shown as adjacent blue/pink bars in separate input-norm and output-norm histograms. Optionally repeat the single top-level norm immediately before the LM head across every attention layer in both graphs; norms inside transformer blocks are never treated as this final norm. Natural numeric ordering is the default (`layer 9` before `layer 10`), with alphabetical ordering available for comparison.
...
- `GET /api/layernorm/analysis?layernorm=...&token_a=0&token_b=1[&attention_layer=...&attention_head=0&attention_operator=qk|ov]` — gain-sorted before/after vectors and optional `WqWkᵀ` or `WoWvᵀ` head analysis; `ov` also returns post-attention output-norm vectors and metrics
- `GET /api/layernorm/attention-sweep?layernorm=...&token_a=0&token_b=1&attention_operator=qk|ov` — paired A/B all-head values for `WqWkᵀ` or `WoWvᵀ`; `ov` also returns paired post-attention-output-norm dot products (`token_id` remains a single-token compatibility parameter)
- `GET /api/layernorm/all-norm-attention-sweep?token_a=0&token_b=1&include_final=false` — paired A/B dot-product sweeps for all recognized block input and output norms, naturally ordered by layer number
