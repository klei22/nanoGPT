...
### Attention iteration microbenchmark

To compare 500 full forward/backward attention iterations on CUDA:

```bash
python scripts/benchmark_hf_attention_speed.py
```

The benchmark uses identical pre-normalized Q/K/V tensors for four paths:
PyTorch ReLU2Max, Triton ReLU2Max, generic explicitly materialized softmax, and
conventional PyTorch SDPA (which selects Flash Attention when supported). It
prints average iteration milliseconds, iterations/second, tokens/second, peak
allocated memory, and speed relative to generic softmax, then writes the same
table to `attention_speed_comparison.txt`. The default A100-oriented shape is
