...
def _repo_root() -> Path:
    """Return the repository root (parent of optimization_and_search/)."""
...
def _make_baseline_metrics(
    ...
    return {
        ...
        "score": score,
        "params": params,
        "peak_torch_allocated_mb": torch_alloc,
        "peak_torch_reserved_mb": torch_reserved,
        "peak_process_gpu_mb": process_gpu,
        "iter_latency_avg": iter_ms,
        "best_iter": best_iter,
        "rankme": rankme,
        "areq": areq,
    }
...
def _compute_cost_delta(
    ...
    cost_map = {
        "params": d_param,
        "vram": d_torch_alloc,
        "torch_allocated": d_torch_alloc,
        "torch_reserved": d_torch_reserved,
        "process_gpu": d_process_gpu,
        "iter": d_iter,
    }
    if efficiency_target not in cost_map:
        raise ValueError(f"Unknown efficiency target: {efficiency_target}")
    return cost_map[efficiency_target]
...
def _get_objective(
    ...
    obj_map = {
        "score": (avg_score, base_score),
        "rankme": (avg_rankme, base_rankme),
        "areq": (avg_areq, base_areq),
    }
    if optimize_target not in obj_map:
        raise ValueError(f"Unknown optimize target: {optimize_target}")
    return obj_map[optimize_target]
...
def run_trial_inproc(cfg: Dict[str, Any]) -> TrialMetrics:
    ...
    metrics = TrialMetrics(
        ...
        num_params=float(tr.raw_model.num_param),
...
def run_trial_subproc(cfg: Dict[str, Any]) -> TrialMetrics:
    root = _repo_root()
    cmd = [sys.executable, str(root / "train.py")] + dict_to_cli(cfg)
...
def _evaluate_candidate(
    ...
    """Evaluate a single candidate configuration. Returns candidate dict or None on failure."""
    ...
    for r in range(args.random_iterations):
        ...
        print(f"[TEST] {label}={value}  seed={cfg_run['seed']}")
        try:
            m = run_fn(cfg_run)
        ...
        seed_runs.append(
            {
                ...
                "peak_torch_allocated_mb": m.torch_alloc_mb,
                "peak_torch_reserved_mb": m.torch_resv_mb,
                "peak_process_gpu_mb": m.process_gpu_mb,
                "iter_latency_ms": m.iter_latency_ms,
                "rankme": m.rankme,
                "areq": m.areq,
            }
        )
    ...
    d_cost = _compute_cost_delta(
        args.efficiency_target, d_param, d_torch_alloc, d_torch_reserved, d_process_gpu, d_iter
    )

    objective_value, baseline_objective = _get_objective(
        args.optimize_target, avg_score, base_score, avg_rankme, base_rankme, avg_areq, base_areq
    )
...
def _update_best(
    best_choice: Optional[Tuple[float, Dict[str, Any]]],
    cand: Dict[str, Any],
) -> Optional[Tuple[float, Dict[str, Any]]]:
    """Update the best candidate if this one has positive and superior efficiency."""
    eff = cand["efficiency"]
    if eff <= 0:
        return best_choice
...
def _resume_or_init_baseline(args, log, run_fn):
    """Returns (baseline_cfg, base metrics, cur_iter)."""
    ...
    if log["iterations"]:
        ...
        bm = last["baseline_metrics"]
        base_loss = bm["loss"]
        base_score = bm["score"]
        base_rankme = bm.get("rankme", float("nan"))
        base_areq = bm.get("areq", float("nan"))
        base_params = bm["params"]
        base_torch_alloc = bm.get(
            "peak_torch_allocated_mb", bm.get("peak_gpu_mb", 0.0)
        )
        base_torch_reserved = bm.get("peak_torch_reserved_mb", 0.0)
        base_process_gpu = bm.get("peak_process_gpu_mb", 0.0)
        base_iter_ms = bm.get("iter_latency_avg", 0.0)
        best_iter = bm["best_iter"]
        cur_iter = last["iter"] + 1
        _apply_overrides(baseline_cfg, args.override_cfg, "resumed baseline_cfg")
    else:
        ...
        m = run_fn(deepcopy(baseline_cfg))
        base_loss = m.loss
        base_score = 1 / math.exp(base_loss)
        base_params = m.num_params
        base_torch_alloc = m.torch_alloc_mb
        base_torch_reserved = m.torch_resv_mb
        base_process_gpu = m.process_gpu_mb
        base_iter_ms = m.iter_latency_ms
        base_rankme = m.rankme
        base_areq = m.areq
        best_iter = m.best_iter
        ...
        log["iterations"].append(
            {
                "iter": -1,
                "baseline_metrics": _make_baseline_metrics(
                    base_loss, base_score, base_params, base_torch_alloc,
                    base_torch_reserved, base_process_gpu, base_iter_ms,
                    best_iter, base_rankme, base_areq,
                ),
    ...
    return (
        baseline_cfg, base_loss, base_score, base_params, base_torch_alloc,
        base_torch_reserved, base_process_gpu, base_iter_ms, base_rankme,
        base_areq, best_iter, cur_iter,
    )


def _apply_chosen_candidate(baseline_cfg, chosen):
    """Update baseline_cfg in-place with the chosen candidate's parameter change."""
...
def main():
    ...
    (
        baseline_cfg, base_loss, base_score, base_params, base_torch_alloc,
        base_torch_reserved, base_process_gpu, base_iter_ms, base_rankme,
        base_areq, base_best_iter, cur_iter,
    ) = _resume_or_init_baseline(args, log, run_fn)

    log["baseline_config"] = deepcopy(baseline_cfg)
    save_log(log_path, log)

    eval_kwargs = lambda: dict(
        run_fn=run_fn,
        args=args,
        base_score=base_score,
        base_rankme=base_rankme,
        base_areq=base_areq,
        base_params=base_params,
        base_torch_alloc=base_torch_alloc,
        base_torch_reserved=base_torch_reserved,
        base_process_gpu=base_process_gpu,
        base_iter_ms=base_iter_ms,
    )
    ...
    while cur_iter < args.num_iterations:
        ...
        best_choice: Optional[Tuple[float, Dict[str, Any]]] = None
        ...
        for pname in args.param_names:
            ...
            if pname == "n_layer":
                ...
                def _nlayer_candidate(dup_idx: int) -> None:
            ...
            if isinstance(base_val, (int, float)):
                for m in range(1, args.iterations + 1):
                    ...
                    cand = _evaluate_candidate(cfg_tmpl, pname, new_val, **eval_kwargs())
                    if cand is not None:
                        candidates.append(cand)
                        best_choice = _update_best(best_choice, cand)
                continue
            ...
            if isinstance(base_val, list):
                ...
                for idx, elem in enumerate(base_val):
                    ...
                    for m in range(1, args.iterations + 1):
                        ...
                        cand = _evaluate_candidate(
                            cfg_tmpl, f"{pname}[{idx}]", new_elem, **eval_kwargs()
                        )
                        if cand is not None:
                            candidates.append(cand)
                            best_choice = _update_best(best_choice, cand)
                    ...
                    log["iterations"].append(
                        {
                            ...
                            "baseline_metrics": _make_baseline_metrics(
                                base_loss, base_score, base_params, base_torch_alloc,
                                base_torch_reserved, base_process_gpu, base_iter_ms,
                                base_best_iter, base_rankme, base_areq,
                            ),
        ...
        _apply_chosen_candidate(baseline_cfg, chosen)
